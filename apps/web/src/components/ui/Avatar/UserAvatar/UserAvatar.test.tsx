...
describe("UserAvatar", () => {
  ...
  it("applies different sizes correctly", () => {
    const { rerender } = render(<UserAvatar {...defaultProps} size="small" />);
    const wrapper = screen.getByTestId("user-avatar");
    expect(wrapper.className).toContain("w-[51px]");
    expect(wrapper.className).toContain("h-[51px]");

    rerender(<UserAvatar {...defaultProps} size="large" />);
  ...
  it("shows notification badge when isNotification is true", () => {
    ...
    rerender(<UserAvatar {...defaultProps} isNotification />);
  ...
  it("applies notification badge sizes correctly", () => {
    const { rerender } = render(
      <UserAvatar {...defaultProps} isNotification size="small" />
    );
    ...
    rerender(<UserAvatar {...defaultProps} isNotification size="large" />);
