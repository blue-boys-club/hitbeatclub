...
export function PaginationPagingPipe(defaultPerPage: number = PAGINATION_DEFAULT_PAGE): Type<PipeTransform> {
	...
	class MixinPaginationPagingPipe implements PipeTransform {
		...
		async transform(value: Record<string, any>): Promise<Record<string, any>> {
			...
			const page: number = this.paginationService.page(
				value?.page ? Number.parseInt(value?.page) : Number.parseInt(rPage.toString()),
			);

			const perPage: number = this.paginationService.perPage(
				Number.parseInt(value?.perPage ? value?.perPage : Number.parseInt(rPerPage.toString())),
			);
